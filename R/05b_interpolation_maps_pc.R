# 05b_interpolation_maps_pc.R — Posterior predictive maps using PC prior model
#
# Same as 05_interpolation_maps.R but loads stage2_matern_pc_results.rds
# and saves figures with _pc suffix.
#
# Run from project root: Rscript R/05b_interpolation_maps_pc.R
...
cat("========================================\n")
cat("Posterior predictive maps (Matern GP + PC priors)\n")
cat("========================================\n")

# ---- 1. Load PC prior posterior ----
cat("Loading Matern GP + PC prior posterior...\n")
...
mu_stn    <- colMeans(exp(eta_psi_draws))
sigma_stn <- colMeans(exp(eta_psi_draws + eta_tau_draws))
xi_stn    <- colMeans(g(eta_phi_draws))
rl100_stn_draws <- exp(eta_psi_draws) +
  exp(eta_psi_draws + eta_tau_draws) / g(eta_phi_draws) *
  ((-log(1 - 1 / M))^(-g(eta_phi_draws)) - 1)
...
p_all <- (p1 + p2) / (p3 + p4) +
  plot_annotation(
    title = "Posterior predictive: extreme precipitation across Andalucia (PC priors)",
    subtitle = sprintf(
      "Matern(5/2) GP + PC priors | %d stations, %d grid points, %d draws | %.3f deg tiles",
      ns, n_pred, n_draws, pred_res),
    theme = theme(plot.title = element_text(face = "bold", size = 14),
                  plot.subtitle = element_text(size = 11, colour = "grey40"))
  )

ggsave("figures/interpolated_maps_pc.pdf", p_all, width = 14, height = 10)
ggsave("figures/interpolated_maps_pc.png", p_all, width = 14, height = 10, dpi = 200)

cat("Saved figures/interpolated_maps_pc.pdf and .png\n")
cat("========================================\n")
cat("Done.\n")
cat("========================================\n")
