# 03d_smooth_matern_pc.R — Stage 2: Matérn(5/2) GP with PC priors
#
# Same model as 03c_smooth_matern.R but with penalized complexity priors
# matching Hazra, Huser & Jóhannesson (2023) book chapter:
#   - mu ~ N(0, 100)
#   - sigma_gp ~ Exp(lambda_s)
#   - phi_gp ~ PC prior: lambda_rho * rho^{-2} * exp(-lambda_rho / rho)
#   - nugget ~ Exp(lambda_nugget)
#
# Quick run: 2 chains x (500 warmup + 500 sampling)
#
# Run from project root: Rscript R/03d_smooth_matern_pc.R
...
# ---- 9. Quick comparison with original Matern model ----
cat("\n  === Comparison with original Matern GP ===\n")
orig <- readRDS("data/stage2_matern_results.rds")

cat(sprintf("  %-15s %10s %10s\n", "Parameter", "Original", "PC prior"))
cat(paste(rep("-", 40), collapse = ""), "\n")
for (nm in c("beta_psi", "beta_tau", "beta_phi",
  ...
  cat(sprintf("  %-15s %10.4f %10.4f\n", nm,
              mean(orig[[nm]]), mean(result[[nm]])))
}

cat("\n========================================\n")
cat("Done. Matern GP + PC priors complete.\n")
cat("========================================\n")
